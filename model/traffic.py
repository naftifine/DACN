...
def plot_kmeans_silhouette_curve(results_rows, out_path, title_prefix="KMeans"):
    ...
    rows = [r for r in results_rows if r.get("algorithm") == "KMeans" and r.get("silhouette") is not None]
    if not rows:
        return
    ...
    for r in rows:
        try:
            p = ast.literal_eval(r.get("params")) if isinstance(r.get("params"), str) else r.get("params")
            parsed.append((int(p.get("k")), int(p.get("n_init", 0)), float(r.get("silhouette"))))
        except Exception:
            continue
    if not parsed:
        return
...
def plot_dbscan_silhouette_heatmap(results_rows, out_path, title_prefix="DBSCAN"):
    ...
    rows = [r for r in results_rows if r.get("algorithm") == "DBSCAN" and r.get("silhouette") is not None]
    if not rows:
        return
    parsed = []
    for r in rows:
        try:
            p = ast.literal_eval(r.get("params")) if isinstance(r.get("params"), str) else r.get("params")
            parsed.append((float(p.get("eps")), int(p.get("min_samples")), float(r.get("silhouette"))))
        except Exception:
            continue
    if not parsed:
        return
...
def plot_feature_count_pipeline(step_counts, out_path, title="Feature/column counts by pipeline step"):
...
def plot_model_selection_scatter(df_models, out_path, title="Model selection: ARI vs Silhouette"):
    ...
    plt.scatter(use["silhouette"], use["ARI"], s=14, c=use["weighted_score"].astype(float), alpha=0.75)
    plt.colorbar(label="weighted_score")
    # Emphasize pareto front + gate pass
    pf = use[(use["pareto_front"] == True) & (use["gate_pass"] == True)]
    if not pf.empty:
        plt.scatter(pf["silhouette"], pf["ARI"], s=35, facecolors="none", edgecolors="black", linewidths=1.1)
    plt.xlabel("Silhouette (internal)")
    plt.ylabel("ARI (external)")
    plt.title(title)
    _savefig(out_path)

def plot_model_selection_table(best_row, out_path, title="Best model (metrics + balance)"):
    ...
    tbl = plt.table(cellText=rows, colLabels=["Field", "Value"], loc="center", cellLoc="left")
...
def _build_X_for_fs(X_all, feature_names_all, fs_feature_sets, fs_method):
    ...
    indices = [feature_names_all.index(f) for f in selected_features if f in feature_names_all]
    if not indices:
...
def _fit_predict_from_row(best_row, X_all, feature_names_all, fs_feature_sets):
    ...
    params = ast.literal_eval(params_raw) if isinstance(params_raw, str) else (params_raw or {})

    X_sel, used_features = _build_X_for_fs(X_all, feature_names_all, fs_feature_sets, fs_method)

    # Build estimator based on algorithm
    if algo == "KMeans":
        est = KMeans(n_clusters=int(params.get("k")), n_init=int(params.get("n_init", 10)), random_state=42)
        labels = est.fit_predict(X_sel)
    elif algo == "MiniBatchKMeans":
        est = MiniBatchKMeans(n_clusters=int(params.get("k")), random_state=42, batch_size=256)
        labels = est.fit_predict(X_sel)
        ...
        if link == "ward":
            ...
            est = AgglomerativeClustering(n_clusters=k, linkage=link, metric="euclidean")
        labels = est.fit_predict(X_sel)
    elif algo == "DBSCAN":
        est = DBSCAN(eps=float(params.get("eps")), min_samples=int(params.get("min_samples")))
        labels = est.fit_predict(X_sel)
    elif algo == "OPTICS":
        est = OPTICS(min_samples=int(params.get("min_samples")), xi=float(params.get("xi")))
        labels = est.fit_predict(X_sel)
    elif algo == "GMM":
        est = GaussianMixture(
...
def plot_best_overall_figures(best_row, X_all, feature_names_all, fs_feature_sets, fig_dir):
    """Create ONLY the 2 main best-overall figures required for the report."""
    try:
        labels, X_sel, used_features = _fit_predict_from_row(best_row, X_all, feature_names_all, fs_feature_sets)
...
# =============================================================
def rank_features_by_model(model, X, y, feature_names, importance_attr="feature_importances_", score_name="importance"):
...
def evaluate_clustering_external(y_true_int, labels):
    ...
    try:
        ...
        y_pred_majority = np.array([cluster_to_class.get(lbl, -1) for lbl in labels_use])
        ...
        if valid_mask.sum() > 0:
            ...
            rmse = np.sqrt(mean_squared_error(y_use[valid_mask], y_pred_majority[valid_mask]))
            results["MAE"] = mae
            results["RMSE"] = rmse
    except Exception:
        pass
...
def _degenerate_reason(row, gate_cfg):
    ...
    if n_clusters is None or (isinstance(n_clusters, (int, np.integer)) and n_clusters <= 1):
...
if y_int is not None:
    ...
    plot_traffic_label_distribution(df, os.path.join(FIG_DIR, "fig01_traffic_label_distribution.png"))
else:
...
plot_missingness_bar(df, numeric_cols, top_n=min(20, len(numeric_cols)), out_path=os.path.join(FIG_DIR, "fig02_missing_ratio_top20_numeric.png"))


# Loại ID, time, geometry,... (nếu có trong numeric_col thì bỏ)
cols_to_exclude = [
    "segmentId", "id", "ID",          # ID
    "timeStamp", "timestamp", "time", # thời gian số
    "sunrise", "sunset",              # thiên văn
]
...
plot_corr_heatmap(df, numeric_feature_cols, out_path=os.path.join(FIG_DIR, "fig03_corr_heatmap_before_filter.png"), max_features=25)
...
plot_corr_heatmap(df, numeric_feature_cols, out_path=os.path.join(FIG_DIR, "fig04_corr_heatmap_after_filter.png"), max_features=25)
...
step_counts = [
    ...
    ("numeric_after_excl", int(len([c for c in df.select_dtypes(include=[np.number]).columns if c not in cols_to_exclude]))),
    ("numeric_no_nan_only", int(len([c for c in [c for c in df.select_dtypes(include=[np.number]).columns if c not in cols_to_exclude] if not df[c].isna().all()]))),
    ("after_var_corr", int(len(numeric_feature_cols))),
]
plot_feature_count_pipeline(step_counts, out_path=os.path.join(FIG_DIR, "fig05_pipeline_feature_counts.png"))
...
et_df = rank_features_by_model(et, X_all, y_int, numeric_feature_cols, importance_attr="feature_importances_", score_name="importance")
...
if _HAS_XGB:
    ...
    xgb_df = rank_features_by_model(xgb, X_all, y_int, numeric_feature_cols, importance_attr="feature_importances_", score_name="importance")
...
def run_clustering_for_feature_set(X_all, feature_names_all, selected_features, df_source, fs_name, y_true_int):
    ...
    for k in k_values:
        for n_init in n_init_list:
            ...
            update_best("partition", "KMeans", {"k": k, "n_init": n_init}, labels, internal, external)
    ...
    for k in k_values:
        ...
        update_best("partition", "MiniBatchKMeans", {"k": k}, labels, internal, external)
    ...
    for link in linkages:
        for k in k_values:
            try:
                ...
                internal = evaluate_clustering_internal(X, labels)
                external = evaluate_clustering_external(y_true_int, labels)
                print(f"Agglomerative link={link}, k={k} -> sil={internal['silhouette']}")
                update_best("hierarchical", f"Agglomerative_{link}", {"k": k}, labels, internal, external)
            except Exception as e:
    ...
    for eps in eps_list:
        for ms in min_samples_list:
            ...
            update_best("density", "DBSCAN", {"eps": eps, "min_samples": ms}, labels, internal, external)
    ...
    for ms in min_samples_list:
        for xi in xi_list:
            ...
            try:
                ...
                update_best("density", "OPTICS", {"min_samples": ms, "xi": xi}, labels, internal, external)
    ...
    for n_comp in k_values:  # dùng chung k lẻ làm số thành phần
        for cov in cov_types:
            try:
                ...
                update_best("model", "GMM", {"n_components": n_comp, "covariance_type": cov}, labels, internal, external)
            except Exception as e:
    ...
    # ================= FS-LEVEL MODEL SELECTION (WEIGHTED + PARETO) =================

    # Gate config (giống best overall)
    gate_cfg = {
    ...
    metrics_df["gate_reason"] = metrics_df.apply(
        lambda row: _degenerate_reason(row, gate_cfg), axis=1
    )
    metrics_df["gate_pass"] = metrics_df["gate_reason"] == ""

    # Normalize metrics
    metrics_df["silhouette_norm"] = _minmax_norm(metrics_df["silhouette"], higher_is_better=True)
    metrics_df["davies_bouldin_norm"] = _minmax_norm(metrics_df["davies_bouldin"], higher_is_better=False)
    metrics_df["ARI_norm"] = _minmax_norm(metrics_df["ARI"], higher_is_better=True)
    metrics_df["NMI_norm"] = _minmax_norm(metrics_df["NMI"], higher_is_better=True)
    metrics_df["RMSE_norm"] = _minmax_norm(metrics_df["RMSE"], higher_is_better=False)

    # Weighted score (GIỐNG BEST OVERALL)
    metrics_df["weighted_score"] = (
        0.25 * metrics_df["silhouette_norm"].fillna(0) +
        0.15 * metrics_df["davies_bouldin_norm"].fillna(0) +
        0.25 * metrics_df["ARI_norm"].fillna(0) +
        0.15 * metrics_df["NMI_norm"].fillna(0) +
        0.20 * metrics_df["RMSE_norm"].fillna(0)
    )
    ...
    metrics_df["pareto_front"] = pareto_front(metrics_df, objectives)

    # Chọn best model cho FS này
    fs_best_df = metrics_df[metrics_df["gate_pass"]].copy()

    if fs_best_df.empty:
        print(f"[WARN] FS={fs_name}: Không có model nào qua gate filter.")
        best_row = metrics_df.iloc[0]
    else:
        fs_best_df = fs_best_df.sort_values(
            ["pareto_front", "weighted_score", "ARI", "silhouette"],
            ascending=[False, False, False, False],
        )
        best_row = fs_best_df.iloc[0]
    ...
    print("  Silhouette:", best_info["silhouette"])

    # Refit model from best_info to get labels (same as best overall logic)
    try:
        labels_best, X_sel, used_features_refit = _fit_predict_from_row(
            best_info, X_all, feature_names_all, fs_feature_sets
        )
    except Exception as e:
        print(f"[WARN] Không thể refit best model cho FS={fs_name}: {e}")
...
for fs_name, feats in fs_feature_sets.items():
    metrics_df, best_info = run_clustering_for_feature_set(X_all, numeric_feature_cols, feats, df, fs_name, y_int)
...
if best_list:
    summary_rows = []
    for b in best_list:
        # `best_info` is a flat dict (a row from metrics_df) produced by FS-level selection.
        # So we extract metrics directly from keys instead of b["metrics_internal"]/b["metrics_external"].
        row = {
            "fs_method": b.get("fs_method"),
            "family": b.get("family"),
            "algorithm": b.get("algorithm"),
            "params": str(b.get("params")),

            # Internal metrics
            "n_clusters": b.get("n_clusters"),
            "silhouette": b.get("silhouette"),
            "davies_bouldin": b.get("davies_bouldin"),
            "calinski_harabasz": b.get("calinski_harabasz"),

            # External metrics
            "ARI": b.get("ARI"),
            "NMI": b.get("NMI"),
            "accuracy_majority": b.get("accuracy_majority"),
            "MAE": b.get("MAE"),
            "RMSE": b.get("RMSE"),

            # Cluster balance / sanity check
            "noise_ratio": b.get("noise_ratio"),
            "largest_cluster_ratio": b.get("largest_cluster_ratio"),
            "min_cluster_size": b.get("min_cluster_size"),
            "largest_cluster_size": b.get("largest_cluster_size"),

            # Selection diagnostics
            "weighted_score": b.get("weighted_score"),
            "pareto_front": b.get("pareto_front"),
            "gate_pass": b.get("gate_pass"),
            "gate_reason": b.get("gate_reason"),
        }
...
if all_metrics_dfs:
    ...
    all_models_df["gate_reason"] = all_models_df.apply(lambda row: _degenerate_reason(row, gate_cfg), axis=1)
    all_models_df["gate_pass"] = all_models_df["gate_reason"] == ""

    # Normalized metrics for scoring
    all_models_df["silhouette_norm"] = _minmax_norm(all_models_df["silhouette"], higher_is_better=True)
    all_models_df["davies_bouldin_norm"] = _minmax_norm(all_models_df["davies_bouldin"], higher_is_better=False)
    all_models_df["ARI_norm"] = _minmax_norm(all_models_df["ARI"], higher_is_better=True)
    all_models_df["NMI_norm"] = _minmax_norm(all_models_df["NMI"], higher_is_better=True)
    all_models_df["RMSE_norm"] = _minmax_norm(all_models_df["RMSE"], higher_is_better=False)
    ...
    plot_best_overall_figures(best_row, X_all, numeric_feature_cols, fs_feature_sets, FIG_DIR)

    # 3rd main report output: model selection table (metrics + balance)
    plot_model_selection_table(best_row, out_path=os.path.join(FIG_DIR, "fig_best_model_selection_table.png"))

    # Optional (high-value): show tradeoff landscape for all tried models
    try:
        plot_model_selection_scatter(all_models_df, out_path=os.path.join(FIG_DIR, "fig06_model_selection_scatter_ARI_vs_Sil.png"))
    except Exception:
        pass
    ...
    print(f"  Internal metrics: silhouette={best_row['silhouette']}, davies_bouldin={best_row['davies_bouldin']}, calinski_harabasz={best_row.get('calinski_harabasz', None)}")
    print(f"  External metrics: ARI={best_row['ARI']}, NMI={best_row['NMI']}, accuracy_majority={best_row['accuracy_majority']}, MAE={best_row['MAE']}, RMSE={best_row['RMSE']}")
    print(f"  Cluster balance: n_clusters={best_row['n_clusters']}, noise_ratio={best_row['noise_ratio']}, largest_cluster_ratio={best_row['largest_cluster_ratio']}, min_cluster_size={best_row['min_cluster_size']}, largest_cluster_size={best_row['largest_cluster_size']}")
